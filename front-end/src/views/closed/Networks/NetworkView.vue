...
<template>
  <div class="p-6 bg-black min-h-screen text-sm text-white relative">
    ...
    <div class="flex items-center justify-between mb-6 border-b border-red-600/30 pb-4">
      <h1 class="text-lg font-bold text-white">Network</h1>
        ...
        <table class="min-w-full text-sm divide-y divide-gray-200">
          <thead class="bg-gray-900/80 text-gray-300 uppercase text-xs font-semibold border-b border-red-600/30">
            <tr>
              ...
              <th class="px-6 py-3 text-left">Talent_id</th><th class="px-6 py-3 text-left">Parent_id(referrer)</th><th class="px-6 py-3 text-left">Depth_level</th><th class="px-6 py-3 text-left">Position</th>
          ...
          <tbody class="divide-y divide-gray-700/50">
            <tr v-for="(item, index) in items" :key="item.id" class="hover:bg-red-900/10 transition duration-150">
              ...
              <td class="px-6 py-4 whitespace-nowrap text-gray-300">{{ item.talent_id }}</td><td class="px-6 py-4 whitespace-nowrap text-gray-300">{{ item.parent_id(referrer) }}</td><td class="px-6 py-4 whitespace-nowrap text-gray-300">{{ item.depth_level }}</td><td class="px-6 py-4 whitespace-nowrap text-gray-300">{{ item.position }}</td>
    ...
    <div class="md:hidden space-y-4">
      <div v-for="(item, index) in items" :key="item.id" class="bg-gradient-to-br from-gray-900/50 to-black border border-gray-700/50 rounded-2xl shadow-md p-4">
        <div class="flex justify-between mb-3">
          <h2 class="font-bold text-white">Network #{{ index + 1 }}</h2>
        ...
        <div class="grid grid-cols-2 gap-y-1 text-sm text-gray-300">
            ...
            <div class="col-span-2">
              <span class="font-medium text-gray-500">Parent_id(referrer):</span>
              {{ item.parent_id(referrer) }}
            </div>
            <div class="col-span-2">
              <span class="font-medium text-gray-500">Depth_level:</span>
              {{ item.depth_level }}
            </div>
            <div class="col-span-2">
    ...
    <!-- Add/Edit Modal -->
    <add-network v-if="showModal && !editMode" :data="selectedItem" @close="showModal=false" @saved="fetchItems"/>
    <edit-network v-if="showModal && editMode" :data="selectedItem" @close="showModal=false" @saved="fetchItems"/>
...
export default {
  components: { AddNetwork, EditNetwork, Loading, DeleteConfirmModal },
  ...
  methods: {
    async fetchItems(page = 1) {
      ...
      try {
        const response = await this.$apiGet('/network', params);
    ...
    viewDetails(id) { 
      this.$router.push({ name: 'Network-detail', params: { id } });
    },
    ...
    // Delete with toast
    async confirmDelete() {
      const res = await this.$apiDelete('/network', this.deleteId);
      if(res) {
        this.$root.$refs.toast.showToast('Network deleted successfully', 'success');
      }

...
<template>
  <div class="p-6 bg-black min-h-screen text-sm text-white relative">
    ...
    <div class="flex items-center justify-between mb-6 border-b border-red-600/30 pb-4">
      <h1 class="text-lg font-bold text-white">UserSubscriptions</h1>
      <button @click="openAddModal" class="bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-lg font-medium shadow-md flex items-center space-x-1 text-sm">
        ...
        <span>Add UserSubscriptions</span>
      </button>
    </div>
        ...
        <table class="min-w-full text-sm divide-y divide-gray-200">
          <thead class="bg-gray-900/80 text-gray-300 uppercase text-xs font-semibold border-b border-red-600/30">
            <tr>
              ...
              <th class="px-6 py-3 text-left">User_id</th><th class="px-6 py-3 text-left">Plan_id</th><th class="px-6 py-3 text-left">Starts_at</th><th class="px-6 py-3 text-left">Ends_at</th><th class="px-6 py-3 text-left">Auto_renew</th>
          ...
          <tbody class="divide-y divide-gray-700/50">
            <tr v-for="(item, index) in items" :key="item.id" class="hover:bg-red-900/10 transition duration-150">
              ...
              <td class="px-6 py-4 whitespace-nowrap text-gray-300">{{ item.user_id }}</td><td class="px-6 py-4 whitespace-nowrap text-gray-300">{{ item.plan_id }}</td><td class="px-6 py-4 whitespace-nowrap text-gray-300">{{ item.starts_at }}</td><td class="px-6 py-4 whitespace-nowrap text-gray-300">{{ item.ends_at }}</td><td class="px-6 py-4 whitespace-nowrap text-gray-300">{{ item.auto_renew }}</td>
    ...
    <div class="md:hidden space-y-4">
      <div v-for="(item, index) in items" :key="item.id" class="bg-gradient-to-br from-gray-900/50 to-black border border-gray-700/50 rounded-2xl shadow-md p-4">
        <div class="flex justify-between mb-3">
          <h2 class="font-bold text-white">UserSubscriptions #{{ index + 1 }}</h2>
        ...
        <div class="grid grid-cols-2 gap-y-1 text-sm text-gray-300">
            ...
            <div class="col-span-2">
              <span class="font-medium text-gray-500">Plan_id:</span>
              {{ item.plan_id }}
            </div>
            <div class="col-span-2">
              <span class="font-medium text-gray-500">Starts_at:</span>
              {{ item.starts_at }}
            </div>
            <div class="col-span-2">
              <span class="font-medium text-gray-500">Ends_at:</span>
              {{ item.ends_at }}
            </div>
            <div class="col-span-2">
              <span class="font-medium text-gray-500">Auto_renew:</span>
              {{ item.auto_renew }}
            </div>
        </div>
      </div>
    ...
    <!-- Add/Edit Modal -->
    <add-usersubscriptions v-if="showModal && !editMode" :data="selectedItem" @close="showModal=false" @saved="fetchItems"/>
    <edit-usersubscriptions v-if="showModal && editMode" :data="selectedItem" @close="showModal=false" @saved="fetchItems"/>

    <!-- Delete Confirmation Modal -->
    <delete-confirm-modal 
      :visible="deleteModalVisible"
      title="Delete UserSubscriptions"
      message="Are you sure you want to delete this UserSubscriptions?"
...
<script>
import AddUserSubscriptions from "./AddUserSubscriptions.vue";
import EditUserSubscriptions from "./EditUserSubscriptions.vue";
import Loading from "@/components/Loading.vue";
import DeleteConfirmModal from "@/components/DeleteConfirmModal.vue";

export default {
  components: { AddUserSubscriptions, EditUserSubscriptions, Loading, DeleteConfirmModal },
  ...
  methods: {
    async fetchItems(page = 1) {
      ...
      try {
        const response = await this.$apiGet('/usersubscriptions', params);
    ...
    viewDetails(id) { 
      this.$router.push({ name: 'UserSubscriptions-detail', params: { id } });
    },
    ...
    // Delete with toast
    async confirmDelete() {
      const res = await this.$apiDelete('/usersubscriptions', this.deleteId);
      if(res) {
        this.$root.$refs.toast.showToast('UserSubscriptions deleted successfully', 'success');
      }

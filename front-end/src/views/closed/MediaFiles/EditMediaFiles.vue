  ...
  <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
    <div class="bg-white rounded-xl shadow-2xl w-full max-w-sm p-6 text-sm">
      ...
      <div class="flex justify-between items-center mb-4 border-b pb-2">
        <h2 class="text-lg font-semibold text-gray-800">Edit MediaFiles</h2>
      ...
      <form @submit.prevent="submitForm" class="space-y-4">
        ...
        <div v-if="data?.file_url">
          <label class="block mb-1 text-sm font-medium text-gray-700">Current File</label>
          <p class="text-xs text-gray-500 break-all">{{ data.file_url }}</p>
        </div>

        <!-- Upload New File -->
        <div>
          <label class="block mb-1 text-sm font-medium text-gray-700">Replace File</label>
          <input 
            type="file"

<template>
  <div class="p-6 bg-black min-h-screen text-sm text-white relative">
    ...
    <div class="flex items-center justify-between mb-6 border-b border-red-600/30 pb-4">
      ...
      <button 
        @click="openAddModal" 
        class="bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-lg font-medium shadow-md flex items-center space-x-1 text-sm transition"
      >
        ...
        <table class="min-w-full text-sm divide-y divide-gray-200">
          ...
          <tbody class="divide-y divide-gray-700/50">
            <tr v-for="(item, index) in items" :key="item.id" class="hover:bg-primary/10 transition duration-150">
              ...
              <td class="px-6 py-4 whitespace-nowrap text-gray-300">{{ item.description }}</td>
              <td class="px-6 py-4 text-center space-x-2">
                ...
                <!-- Edit -->
                <button @click="editItem(item)" class="text-blue-400 hover:text-blue-300 transition duration-150 transition" title="Edit Role">
                  <i class="fas fa-edit"></i>
                </button>

                <!-- Delete -->
                <button @click="openDeleteModal(item.id)" class="text-red-500 hover:text-red-400 transition duration-150 transition" title="Delete Role">
      ...
      <div v-for="(item, index) in items" :key="item.id" class="bg-gradient-to-br from-gray-900/50 to-black border border-gray-700/50 rounded-2xl shadow-md p-4">
        <div class="flex justify-between mb-3">
          ...
          <div class="flex gap-3 text-sm">
            ...
            <button @click="editItem(item)" class="text-blue-400 hover:text-blue-300 transition duration-150 transition"><i class="fas fa-edit"></i></button>
            <button @click="openDeleteModal(item.id)" class="text-red-500 hover:text-red-400 transition duration-150 transition"><i class="fas fa-trash"></i></button>
          </div>
        </div>
    ...
    <div class="flex items-center justify-between mt-6 text-sm text-gray-500">
      ...
      <div class="flex items-center gap-2">
        <button 
          ...
          class="px-3 py-1 border border-gray-600 text-gray-300 rounded-lg hover:bg-gray-800/50 transition duration-150 disabled:opacity-50 disabled:cursor-not-allowed transition duration-150"
        >
          ← Previous
        </button>

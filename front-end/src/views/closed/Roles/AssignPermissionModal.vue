<template>
  <div class="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-[110] p-4">
    <div class="bg-gradient-to-br from-gray-900/95 to-black rounded-[2.5rem] shadow-2xl w-full max-w-md overflow-hidden animate-pop-in border border-red-600/30">
      ...
      <div class="p-8">
        ...
        <div class="flex justify-between items-start mb-6">
          <div>
            <h2 class="text-2xl font-black text-white leading-none">Assign Permission</h2>
          ...
          <button @click="$emit('close')" class="w-8 h-8 rounded-full bg-gray-800 flex items-center justify-center text-gray-400 hover:text-primary">
            <i class="fas fa-times text-xs"></i>
          </button>
        </div>

        <!-- ROLE INFO -->
        <div class="mb-8 p-4 bg-gray-800/50 rounded-2xl flex items-center gap-4 border border-gray-700/50">
          <div class="w-12 h-12 rounded-xl bg-slate-900 text-primary flex items-center justify-center font-black">
            {{ role.name }}
          </div>
          <div>
            <p class="text-[10px] font-black uppercase tracking-widest text-gray-400">Target Role</p>
            <p class="text-sm font-black text-white uppercase tracking-tighter">{{ role.name }}</p>
          </div>
        </div>
        ...
        <form @submit.prevent="submitAssignment" class="space-y-6">
          <div class="space-y-2">
            ...
            <div class="max-h-64 overflow-y-auto border border-gray-700/50 rounded-2xl p-3 bg-gray-800/50">
              <div v-for="p in permissions" :key="p.id" class="flex items-center gap-2 mb-2">
                ...
                <span class="text-sm font-bold text-gray-300">{{ p.code }}</span>
              </div>
            </div>
          </div>

          <div class="flex gap-3 pt-4">
            <button type="button" @click="$emit('close')" class="flex-1 px-6 py-4 rounded-2xl font-black text-[10px] uppercase tracking-widest text-gray-400 hover:bg-gray-800/50">
              Cancel
            </button>

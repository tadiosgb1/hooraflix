...
<template>
  <div class="p-6 bg-black min-h-screen text-sm text-white relative">
    ...
    <div class="flex items-center justify-between mb-6 border-b border-red-600/30 pb-4">
      <h1 class="text-lg font-bold text-white">ContentPricing</h1>
        ...
        <table class="min-w-full text-sm divide-y divide-gray-200">
          <thead class="bg-gray-900/80 text-gray-300 uppercase text-xs font-semibold border-b border-red-600/30">
            <tr>
              ...
              <th class="px-6 py-3 text-left">Content_id</th><th class="px-6 py-3 text-left">Pricing_plan_id</th><th class="px-6 py-3 text-left">Is_active</th><th class="px-6 py-3 text-left">Priority</th>
          ...
          <tbody class="divide-y divide-gray-700/50">
            <tr v-for="(item, index) in items" :key="item.id" class="hover:bg-red-900/10 transition duration-150">
              ...
              <td class="px-6 py-4 whitespace-nowrap text-gray-300">{{ item.content_id }}</td><td class="px-6 py-4 whitespace-nowrap text-gray-300">{{ item.pricing_plan_id }}</td><td class="px-6 py-4 whitespace-nowrap text-gray-300">{{ item.is_active }}</td><td class="px-6 py-4 whitespace-nowrap text-gray-300">{{ item.priority }}</td>
    ...
    <div class="md:hidden space-y-4">
      <div v-for="(item, index) in items" :key="item.id" class="bg-gradient-to-br from-gray-900/50 to-black border border-gray-700/50 rounded-2xl shadow-md p-4">
        <div class="flex justify-between mb-3">
          <h2 class="font-bold text-white">ContentPricing #{{ index + 1 }}</h2>
        ...
        <div class="grid grid-cols-2 gap-y-1 text-sm text-gray-300">
            ...
            <div class="col-span-2">
              <span class="font-medium text-gray-500">Pricing_plan_id:</span>
              {{ item.pricing_plan_id }}
            </div>
            <div class="col-span-2">
              <span class="font-medium text-gray-500">Is_active:</span>
              {{ item.is_active }}
            </div>
            <div class="col-span-2">
    ...
    <delete-confirm-modal 
      ...
      title="Delete ContentPricing"
      message="Are you sure you want to delete this ContentPricing?"
...
<script>
import AddContentPricing from "./AddContentPricing.vue";
import EditContentPricing from "./EditContentPricing.vue";
import Loading from "@/components/Loading.vue";
import DeleteConfirmModal from "@/components/DeleteConfirmModal.vue";

export default {
  components: { AddContentPricing, EditContentPricing, Loading, DeleteConfirmModal },
  ...
  methods: {
    ...
    viewDetails(id) { 
      this.$router.push({ name: 'ContentPricing-detail', params: { id } });
    },
    ...
    // Delete with toast
    async confirmDelete() {
      const res = await this.$apiDelete('/contentpricing', this.deleteId);
      if(res) {
        this.$root.$refs.toast.showToast('ContentPricing deleted successfully', 'success');
      }

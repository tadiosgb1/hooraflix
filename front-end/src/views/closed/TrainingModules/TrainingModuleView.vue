...
<template>
  <div class="p-6 bg-black min-h-screen text-sm text-white relative">
    <!-- Loading -->
    <Loading :visible="loading" message="Loading trainingModule..." />

    <!-- Page Header -->
    <div class="flex items-center justify-between mb-6 border-b border-red-600/30 pb-4">
      <h1 class="text-lg font-bold text-white">TrainingModule</h1>
        ...
        <table class="min-w-full text-sm divide-y divide-gray-200">
          <thead class="bg-gray-900/80 text-gray-300 uppercase text-xs font-semibold border-b border-red-600/30">
            <tr>
              ...
              <th class="px-6 py-3 text-left">Title</th><th class="px-6 py-3 text-left">Course_content_url</th><th class="px-6 py-3 text-left">Price</th><th class="px-6 py-3 text-left">Instructor_id</th><th class="px-6 py-3 text-left">Certification_eligible</th>
          ...
          <tbody class="divide-y divide-gray-700/50">
            <tr v-for="(item, index) in items" :key="item.id" class="hover:bg-red-900/10 transition duration-150">
              ...
              <td class="px-6 py-4 whitespace-nowrap text-gray-300">{{ item.title }}</td><td class="px-6 py-4 whitespace-nowrap text-gray-300">{{ item.course_content_url }}</td><td class="px-6 py-4 whitespace-nowrap text-gray-300">{{ item.price }}</td><td class="px-6 py-4 whitespace-nowrap text-gray-300">{{ item.instructor_id }}</td><td class="px-6 py-4 whitespace-nowrap text-gray-300">{{ item.certification_eligible }}</td>
    ...
    <div class="md:hidden space-y-4">
      <div v-for="(item, index) in items" :key="item.id" class="bg-gradient-to-br from-gray-900/50 to-black border border-gray-700/50 rounded-2xl shadow-md p-4">
        <div class="flex justify-between mb-3">
          <h2 class="font-bold text-white">TrainingModule #{{ index + 1 }}</h2>
        ...
        <div class="grid grid-cols-2 gap-y-1 text-sm text-gray-300">
            ...
            <div class="col-span-2">
              <span class="font-medium text-gray-500">Course_content_url:</span>
              {{ item.course_content_url }}
            </div>
            <div class="col-span-2">
            ...
            <div class="col-span-2">
              <span class="font-medium text-gray-500">Instructor_id:</span>
              {{ item.instructor_id }}
            </div>
            <div class="col-span-2">
              <span class="font-medium text-gray-500">Certification_eligible:</span>
              {{ item.certification_eligible }}
            </div>
        </div>
      </div>
    ...
    <!-- Add/Edit Modal -->
    <add-trainingmodule v-if="showModal && !editMode" :data="selectedItem" @close="showModal=false" @saved="fetchItems"/>
    <edit-trainingmodule v-if="showModal && editMode" :data="selectedItem" @close="showModal=false" @saved="fetchItems"/>

    <!-- Delete Confirmation Modal -->
    <delete-confirm-modal 
      :visible="deleteModalVisible"
      title="Delete TrainingModule"
      message="Are you sure you want to delete this trainingModule?"
...
<script>
import AddTrainingModule from "./AddTrainingModule.vue";
import EditTrainingModule from "./EditTrainingModule.vue";
import Loading from "@/components/Loading.vue";
import DeleteConfirmModal from "@/components/DeleteConfirmModal.vue";

export default {
  components: { AddTrainingModule, EditTrainingModule, Loading, DeleteConfirmModal },
  ...
  methods: {
    async fetchItems(page = 1) {
      ...
      try {
        const response = await this.$apiGet('/trainingmodule', params);
    ...
    viewDetails(id) { 
      this.$router.push({ name: 'TrainingModule-detail', params: { id } });
    },
    ...
    // Delete with toast
    async confirmDelete() {
      const res = await this.$apiDelete('/trainingmodule', this.deleteId);
      if(res) {
        this.$root.$refs.toast.showToast('TrainingModule deleted successfully', 'success');
      }

...
<template>
  <div class="p-6 bg-black min-h-screen text-sm text-white">
    ...
    <div class="flex items-center justify-between mb-6 border-b border-red-600/30 pb-4">
      <h1 class="text-lg font-bold text-white">TrainingModule Detail</h1>
    </div>
    ...
    <div class="bg-gradient-to-br from-gray-900/50 to-black overflow-hidden rounded-2xl border border-gray-700/50 p-4 shadow-md hidden md:block space-y-2">
      ...
      <div><strong>Title:</strong> {{ item.title }}</div><div><strong>Course_content_url:</strong> {{ item.course_content_url }}</div><div><strong>Price:</strong> {{ item.price }}</div><div><strong>Instructor_id:</strong> {{ item.instructor_id }}</div><div><strong>Certification_eligible:</strong> {{ item.certification_eligible }}</div>
    </div>
...
export default {
  ...
  async mounted() {
    ...
    try {
      const response = await this.$apiGetById('/trainingmodule', id);
      this.item = response || {};
    } catch (error) {
      console.error(error);
    } finally {

...
<template>
  <div class="p-6 bg-black min-h-screen text-sm text-white relative">
    ...
    <div class="flex items-center justify-between mb-6 border-b border-red-600/30 pb-4">
      <h1 class="text-lg font-bold text-white">AdCampaign</h1>
        ...
        <table class="min-w-full text-sm divide-y divide-gray-200">
          <thead class="bg-gray-900/80 text-gray-300 uppercase text-xs font-semibold border-b border-red-600/30">
            <tr>
              ...
              <th class="px-6 py-3 text-left">Advertiser_id</th><th class="px-6 py-3 text-left">Title</th><th class="px-6 py-3 text-left">Ad_video_url</th><th class="px-6 py-3 text-left">Target_demographic</th><th class="px-6 py-3 text-left">Total_budget</th><th class="px-6 py-3 text-left">Status</th>
          ...
          <tbody class="divide-y divide-gray-700/50">
            <tr v-for="(item, index) in items" :key="item.id" class="hover:bg-red-900/10 transition duration-150">
              ...
              <td class="px-6 py-4 whitespace-nowrap text-gray-300">{{ item.advertiser_id }}</td><td class="px-6 py-4 whitespace-nowrap text-gray-300">{{ item.title }}</td><td class="px-6 py-4 whitespace-nowrap text-gray-300">{{ item.ad_video_url }}</td><td class="px-6 py-4 whitespace-nowrap text-gray-300">{{ item.target_demographic }}</td><td class="px-6 py-4 whitespace-nowrap text-gray-300">{{ item.total_budget }}</td><td class="px-6 py-4 whitespace-nowrap text-gray-300">{{ item.status }}</td>
    ...
    <div class="md:hidden space-y-4">
      <div v-for="(item, index) in items" :key="item.id" class="bg-gradient-to-br from-gray-900/50 to-black border border-gray-700/50 rounded-2xl shadow-md p-4">
        <div class="flex justify-between mb-3">
          <h2 class="font-bold text-white">AdCampaign #{{ index + 1 }}</h2>
        ...
        <div class="grid grid-cols-2 gap-y-1 text-sm text-gray-300">
            ...
            <div class="col-span-2">
              <span class="font-medium text-gray-500">Advertiser_id:</span>
              {{ item.advertiser_id }}
            </div>
            <div class="col-span-2">
            ...
            <div class="col-span-2">
              <span class="font-medium text-gray-500">Ad_video_url:</span>
              {{ item.ad_video_url }}
            </div>
            <div class="col-span-2">
              <span class="font-medium text-gray-500">Target_demographic:</span>
              {{ item.target_demographic }}
            </div>
            <div class="col-span-2">
              <span class="font-medium text-gray-500">Total_budget:</span>
              {{ item.total_budget }}
            </div>
            <div class="col-span-2">
    ...
    <!-- Add/Edit Modal -->
    <add-adcampaign v-if="showModal && !editMode" :data="selectedItem" @close="showModal=false" @saved="fetchItems"/>
    <edit-adcampaign v-if="showModal && editMode" :data="selectedItem" @close="showModal=false" @saved="fetchItems"/>

    <!-- Delete Confirmation Modal -->
    <delete-confirm-modal 
      :visible="deleteModalVisible"
      title="Delete AdCampaign"
      message="Are you sure you want to delete this adCampaign?"
...
<script>
import AddAdCampaign from "./AddAdCampaign.vue";
import EditAdCampaign from "./EditAdCampaign.vue";
import Loading from "@/components/Loading.vue";
import DeleteConfirmModal from "@/components/DeleteConfirmModal.vue";

export default {
  components: { AddAdCampaign, EditAdCampaign, Loading, DeleteConfirmModal },
  ...
  methods: {
    async fetchItems(page = 1) {
      ...
      try {
        const response = await this.$apiGet('/adcampaign', params);
    ...
    viewDetails(id) { 
      this.$router.push({ name: 'AdCampaign-detail', params: { id } });
    },
    ...
    // Delete with toast
    async confirmDelete() {
      const res = await this.$apiDelete('/adcampaign', this.deleteId);
      if(res) {
        this.$root.$refs.toast.showToast('AdCampaign deleted successfully', 'success');
      }

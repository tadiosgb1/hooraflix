...
<template>
  <div class="p-6 bg-gray-50 min-h-screen text-sm text-gray-800 relative">
    ...
    <div class="flex items-center justify-between mb-6 border-b pb-4 border-gray-200">
      <h1 class="text-lg font-bold text-gray-800">AdCampaign</h1>
        ...
        <table class="min-w-full text-sm divide-y divide-gray-200">
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-for="(item, index) in items" :key="item.id" class="hover:bg-green-50 transition duration-150">
              ...
              <td class="px-6 py-4 whitespace-nowrap">{{ item.advertiser_id }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.title }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.ad_video_url }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.target_demographic }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.total_budget }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.status }}</td>
    ...
    <div class="md:hidden space-y-4">
      <div v-for="(item, index) in items" :key="item.id" class="bg-white border border-gray-200 rounded-xl shadow p-4">
        <div class="flex justify-between mb-3">
          <h2 class="font-bold text-gray-800">AdCampaign #{{ index + 1 }}</h2>
        ...
        <div class="grid grid-cols-2 gap-y-1 text-sm text-gray-700">
            ...
            <div class="col-span-2">
              <span class="font-medium text-gray-600">Advertiser_id:</span>
              {{ item.advertiser_id }}
            </div>
            <div class="col-span-2">
            ...
            <div class="col-span-2">
              <span class="font-medium text-gray-600">Ad_video_url:</span>
              {{ item.ad_video_url }}
            </div>
            <div class="col-span-2">
              <span class="font-medium text-gray-600">Target_demographic:</span>
              {{ item.target_demographic }}
            </div>
            <div class="col-span-2">
              <span class="font-medium text-gray-600">Total_budget:</span>
              {{ item.total_budget }}
            </div>
            <div class="col-span-2">

...
<template>
  <div class="p-6 bg-black min-h-screen text-sm text-white relative">
    ...
    <div class="flex items-center justify-between mb-6 border-b border-red-600/30 pb-4">
      <h1 class="text-lg font-bold text-white">Content</h1>
        ...
        <table class="min-w-full text-sm divide-y divide-gray-700/50">
          ...
          <tbody class="divide-y divide-gray-700/50">
            <tr v-for="(item, index) in items" :key="item.id" class="hover:bg-red-900/10 transition duration-150">
              ...
              <td class="px-6 py-4 whitespace-nowrap text-gray-300">{{ item.title }}</td><td class="px-6 py-4 whitespace-nowrap text-gray-300">{{ item.description }}</td><td class="px-6 py-4 whitespace-nowrap text-gray-300">{{ item.content_type }}</td><td class="px-6 py-4 whitespace-nowrap text-gray-300">{{ item.release_date }}</td><td class="px-6 py-4 whitespace-nowrap text-gray-300">{{ item.duration }}</td><td class="px-6 py-4 whitespace-nowrap text-gray-300">{{ item.language }}</td><td class="px-6 py-4 whitespace-nowrap text-gray-300">{{ item.country }}</td><td class="px-6 py-4 whitespace-nowrap text-gray-300">{{ item.rating }}</td>
    ...
    <div class="md:hidden space-y-4">
      <div v-for="(item, index) in items" :key="item.id" class="bg-gradient-to-br from-gray-900/50 to-black border border-gray-700/50 rounded-2xl shadow-md p-4">
        <div class="flex justify-between mb-3">
          <h2 class="font-bold text-white">Content #{{ index + 1 }}</h2>
        ...
        <div class="grid grid-cols-2 gap-y-1 text-sm text-gray-500">
            ...
            <div class="col-span-2">
              <span class="font-medium text-gray-500">Content_type:</span>
              {{ item.content_type }}
            </div>
            <div class="col-span-2">
              <span class="font-medium text-gray-500">Release_date:</span>
              {{ item.release_date }}
            </div>
            <div class="col-span-2">

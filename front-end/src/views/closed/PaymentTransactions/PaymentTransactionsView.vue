...
<template>
  <div class="p-6 bg-black min-h-screen text-sm text-white relative">
    ...
    <div class="flex items-center justify-between mb-6 border-b border-red-600/30 pb-4">
      <h1 class="text-lg font-bold text-white">PaymentTransactions</h1>
      <button @click="openAddModal" class="bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-lg font-medium shadow-md flex items-center space-x-1 text-sm transition">
        <svg class="h-4 w-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" />
        </svg>
        <span>Add PaymentTransactions</span>
      </button>
    </div>
        ...
        <table class="min-w-full text-sm divide-y divide-gray-700/50">
          <thead class="bg-gray-900/80 text-gray-300 uppercase text-xs font-semibold border-b border-red-600/30">
            <tr>
              ...
              <th class="px-6 py-3 text-left">User_id</th><th class="px-6 py-3 text-left">Content_id</th><th class="px-6 py-3 text-left">Amount</th><th class="px-6 py-3 text-left">Payment_type</th><th class="px-6 py-3 text-left">Currency</th><th class="px-6 py-3 text-left">Reference_id</th><th class="px-6 py-3 text-left">External_transaction_id</th><th class="px-6 py-3 text-left">Metadata</th>
          ...
          <tbody class="divide-y divide-gray-700/50">
            <tr v-for="(item, index) in items" :key="item.id" class="hover:bg-red-900/10 transition duration-150">
              ...
              <td class="px-6 py-4 whitespace-nowrap text-gray-300">{{ item.user_id }}</td><td class="px-6 py-4 whitespace-nowrap text-gray-300">{{ item.content_id }}</td><td class="px-6 py-4 whitespace-nowrap text-gray-300">{{ item.amount }}</td><td class="px-6 py-4 whitespace-nowrap text-gray-300">{{ item.payment_type }}</td><td class="px-6 py-4 whitespace-nowrap text-gray-300">{{ item.currency }}</td><td class="px-6 py-4 whitespace-nowrap text-gray-300">{{ item.reference_id }}</td><td class="px-6 py-4 whitespace-nowrap text-gray-300">{{ item.external_transaction_id }}</td><td class="px-6 py-4 whitespace-nowrap text-gray-300">{{ item.metadata }}</td>
              <td class="px-6 py-4 text-center space-x-3">
                <button @click="viewDetails(item.id)" class="text-red-400 hover:text-red-300 transition duration-150"><i class="fas fa-eye"></i></button>
            ...
            <tr v-if="items.length === 0">
              <td colspan="10" class="text-center py-6 text-gray-500 italic">No data found.</td>
            </tr>
          </tbody>
        </table>
      </div>
    ...
    <div class="md:hidden space-y-4">
      <div v-for="(item, index) in items" :key="item.id" class="bg-gradient-to-br from-gray-900/50 to-black border border-gray-700/50 rounded-2xl shadow-md p-4">
        <div class="flex justify-between mb-3">
          <h2 class="font-bold text-white">PaymentTransactions #{{ index + 1 }}</h2>
        ...
        <div class="grid grid-cols-2 gap-y-1 text-sm text-gray-300">
          ...
          <div class="col-span-2">
            <span class="font-medium text-gray-500">Payment_type:</span>
            {{ item.payment_type }}
          </div>
          <div class="col-span-2">
          ...
          <div class="col-span-2">
            <span class="font-medium text-gray-500">Reference_id:</span>
            {{ item.reference_id }}
          </div>
          <div class="col-span-2">
            <span class="font-medium text-gray-500">External_transaction_id:</span>
            {{ item.external_transaction_id }}
          </div>
          <div class="col-span-2">
            <span class="font-medium text-gray-500">Metadata:</span>
            {{ item.metadata }}
          </div>
        </div>
      </div>
    ...
    <!-- Add/Edit Modal -->
    <AddPaymentTransactions v-if="showModal && !editMode" :data="selectedItem" @close="showModal=false" @saved="fetchItems"/>
    <EditPaymentTransactions v-if="showModal && editMode" :data="selectedItem" @close="showModal=false" @saved="fetchItems"/>

    <!-- Delete Confirmation Modal -->
    <delete-confirm-modal 
      :visible="deleteModalVisible"
      title="Delete PaymentTransactions"
      message="Are you sure you want to delete this PaymentTransactions?"
...
<script>
import AddPaymentTransactions from "./AddPaymentTransactions.vue";
import EditPaymentTransactions from "./EditPaymentTransactions.vue";
import Loading from "@/components/Loading.vue";
import DeleteConfirmModal from "@/components/DeleteConfirmModal.vue";

export default {
  components: { AddPaymentTransactions, EditPaymentTransactions, Loading, DeleteConfirmModal },
  ...
  methods: {
    async fetchItems(page = 1) {
      ...
      try {
        const response = await this.$apiGet('/paymenttransactions', params);
    ...
    viewDetails(id) { 
      this.$router.push({ name: 'PaymentTransactions-detail', params: { id } });
    },
    ...
    // Delete with toast
    async confirmDelete() {
      const res = await this.$apiDelete('/paymenttransactions', this.deleteId);
      if(res) {
        this.$root.$refs.toast.showToast('PaymentTransactions deleted successfully', 'success');
      }

...
<template>
  <div class="p-6 bg-black min-h-screen text-sm text-white relative">
    <!-- Loading -->
    <Loading :visible="loading" message="Loading RevenueShares..." />

    <!-- Page Header -->
    <div class="flex items-center justify-between mb-6 border-b border-red-600/30 pb-4">
      <h1 class="text-lg font-bold text-white">RevenueShares</h1>
        ...
        <table class="min-w-full text-sm divide-y divide-gray-700/50">
          <thead class="bg-gray-900/80 text-gray-300 uppercase text-xs font-semibold border-b border-red-600/30">
            <tr>
              ...
              <th class="px-6 py-3 text-left">Content_id</th><th class="px-6 py-3 text-left">Creator_id</th><th class="px-6 py-3 text-left">Percentage</th><th class="px-6 py-3 text-left">Fixed_amount</th><th class="px-6 py-3 text-left">Revenue_type</th>
          ...
          <tbody class="divide-y divide-gray-700/50">
            <tr v-for="(item, index) in items" :key="item.id" class="hover:bg-red-900/10 transition duration-150">
              ...
              <td class="px-6 py-4 whitespace-nowrap text-gray-300">{{ item.content_id }}</td><td class="px-6 py-4 whitespace-nowrap text-gray-300">{{ item.creator_id }}</td><td class="px-6 py-4 whitespace-nowrap text-gray-300">{{ item.percentage }}</td><td class="px-6 py-4 whitespace-nowrap text-gray-300">{{ item.fixed_amount }}</td><td class="px-6 py-4 whitespace-nowrap text-gray-300">{{ item.revenue_type }}</td>
    ...
    <div class="md:hidden space-y-4">
      <div v-for="(item, index) in items" :key="item.id" class="bg-gradient-to-br from-gray-900/50 to-black border border-gray-700/50 rounded-2xl shadow-md p-4">
        <div class="flex justify-between mb-3">
          <h2 class="font-bold text-white">RevenueShares #{{ index + 1 }}</h2>
        ...
        <div class="grid grid-cols-2 gap-y-1 text-sm text-gray-300">
          ...
          <div class="col-span-2">
            <span class="font-medium text-gray-500">Percentage:</span>
            {{ item.percentage }}
          </div>
          <div class="col-span-2">
            <span class="font-medium text-gray-500">Fixed_amount:</span>
            {{ item.fixed_amount }}
          </div>
          <div class="col-span-2">
            <span class="font-medium text-gray-500">Revenue_type:</span>
            {{ item.revenue_type }}
          </div>
        </div>
      </div>
    ...
    <!-- Add/Edit Modal -->
    <AddRevenueShares v-if="showModal && !editMode" :data="selectedItem" @close="showModal=false" @saved="fetchItems"/>
    <EditRevenueShares v-if="showModal && editMode" :data="selectedItem" @close="showModal=false" @saved="fetchItems"/>

    <!-- Delete Confirmation Modal -->
    <delete-confirm-modal 
      :visible="deleteModalVisible"
      title="Delete RevenueShares"
      message="Are you sure you want to delete this RevenueShares?"
...
<script>
import AddRevenueShares from "./AddRevenueShares.vue";
import EditRevenueShares from "./EditRevenueShares.vue";
import Loading from "@/components/Loading.vue";
import DeleteConfirmModal from "@/components/DeleteConfirmModal.vue";

export default {
  components: { AddRevenueShares, EditRevenueShares, Loading, DeleteConfirmModal },
  ...
  methods: {
    async fetchItems(page = 1) {
      ...
      try {
        const response = await this.$apiGet('/revenue-shares', params);
    ...
    viewDetails(id) { 
      this.$router.push({ name: 'RevenueShares-detail', params: { id } });
    },
    ...
    // Delete with toast
    async confirmDelete() {
      const res = await this.$apiDelete('/revenueshares', this.deleteId);
      if(res) {
        this.$root.$refs.toast.showToast('RevenueShares deleted successfully', 'success');
      }

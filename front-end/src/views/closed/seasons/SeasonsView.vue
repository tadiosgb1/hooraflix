...
<template>
  <div class="p-6 bg-black min-h-screen text-sm text-white relative">
    <!-- Loading -->
    <Loading :visible="loading" message="Loading seasons..." />

    <!-- Page Header -->
    <div class="flex items-center justify-between mb-6 border-b border-red-600/30 pb-4">
      <h1 class="text-lg font-bold text-white">Seasons</h1>
        ...
        <table class="min-w-full text-sm divide-y divide-gray-200">
          <thead class="bg-gray-900/80 text-gray-300 uppercase text-xs font-semibold border-b border-red-600/30">
            <tr>
              ...
              <th class="px-6 py-3 text-left">Series_id</th><th class="px-6 py-3 text-left">Season_number</th>
          ...
          <tbody class="divide-y divide-gray-700/50">
            <tr v-for="(item, index) in items" :key="item.id" class="hover:bg-red-900/10 transition duration-150">
              ...
              <td class="px-6 py-4 whitespace-nowrap text-gray-300">{{ item.series_id }}</td><td class="px-6 py-4 whitespace-nowrap text-gray-300">{{ item.season_number }}</td>
    ...
    <div class="md:hidden space-y-4">
      <div v-for="(item, index) in items" :key="item.id" class="bg-gradient-to-br from-gray-900/50 to-black border border-gray-700/50 rounded-2xl shadow-md p-4">
        <div class="flex justify-between mb-3">
          <h2 class="font-bold text-white">Seasons #{{ index + 1 }}</h2>
        ...
        <div class="grid grid-cols-2 gap-y-1 text-sm text-gray-300">
            ...
            <div class="col-span-2">
              <span class="font-medium text-gray-500">Series_id:</span>
              {{ item.series_id }}
            </div>
            <div class="col-span-2">
              <span class="font-medium text-gray-500">Season_number:</span>
              {{ item.season_number }}
            </div>
        </div>
      </div>
    ...
    <!-- Add/Edit Modal -->
    <add-seasons v-if="showModal && !editMode" :data="selectedItem" @close="showModal=false" @saved="fetchItems"/>
    <edit-seasons v-if="showModal && editMode" :data="selectedItem" @close="showModal=false" @saved="fetchItems"/>

    <!-- Delete Confirmation Modal -->
    <delete-confirm-modal 
      :visible="deleteModalVisible"
      title="Delete Seasons"
      message="Are you sure you want to delete this seasons?"
...
<script>
import AddSeasons from "./AddSeasons.vue";
import EditSeasons from "./EditSeasons.vue";
import Loading from "@/components/Loading.vue";
import DeleteConfirmModal from "@/components/DeleteConfirmModal.vue";

export default {
  components: { AddSeasons, EditSeasons, Loading, DeleteConfirmModal },
  ...
  methods: {
    ...
    viewDetails(id) { 
      this.$router.push({ name: 'Seasons-detail', params: { id } });
    },
    ...
    // Delete with toast
    async confirmDelete() {
      const res = await this.$apiDelete('/seasons', this.deleteId);
      if(res) {
        this.$root.$refs.toast.showToast('Seasons deleted successfully', 'success');
      }

...
<template>
  <div class="p-6 bg-black min-h-screen text-sm text-white relative">
    <!-- Loading -->
    <Loading :visible="loading" message="Loading talentRank..." />

    <!-- Page Header -->
    <div class="flex items-center justify-between mb-6 border-b border-red-600/30 pb-4">
      <h1 class="text-lg font-bold text-white">TalentRank</h1>
        ...
        <table class="min-w-full text-sm divide-y divide-gray-200">
          <thead class="bg-gray-900/80 text-gray-300 uppercase text-xs font-semibold border-b border-red-600/30">
            <tr>
              ...
              <th class="px-6 py-3 text-left">Talent_id</th><th class="px-6 py-3 text-left">Current_rank</th><th class="px-6 py-3 text-left">Total_referrals</th><th class="px-6 py-3 text-left">Total_points</th>
          ...
          <tbody class="divide-y divide-gray-700/50">
            <tr v-for="(item, index) in items" :key="item.id" class="hover:bg-red-900/10 transition duration-150">
              ...
              <td class="px-6 py-4 whitespace-nowrap text-gray-300">{{ item.talent_id }}</td><td class="px-6 py-4 whitespace-nowrap text-gray-300">{{ item.current_rank }}</td><td class="px-6 py-4 whitespace-nowrap text-gray-300">{{ item.total_referrals }}</td><td class="px-6 py-4 whitespace-nowrap text-gray-300">{{ item.total_points }}</td>
    ...
    <div class="md:hidden space-y-4">
      <div v-for="(item, index) in items" :key="item.id" class="bg-gradient-to-br from-gray-900/50 to-black border border-gray-700/50 rounded-2xl shadow-md p-4">
        <div class="flex justify-between mb-3">
          <h2 class="font-bold text-white">TalentRank #{{ index + 1 }}</h2>
        ...
        <div class="grid grid-cols-2 gap-y-1 text-sm text-gray-300">
            ...
            <div class="col-span-2">
              <span class="font-medium text-gray-500">Talent_id:</span>
              {{ item.talent_id }}
            </div>
            <div class="col-span-2">
              <span class="font-medium text-gray-500">Current_rank:</span>
              {{ item.current_rank }}
            </div>
            <div class="col-span-2">
              <span class="font-medium text-gray-500">Total_referrals:</span>
              {{ item.total_referrals }}
            </div>
            <div class="col-span-2">
              <span class="font-medium text-gray-500">Total_points:</span>
              {{ item.total_points }}
            </div>
        </div>
      </div>
    ...
    <!-- Add/Edit Modal -->
    <add-talentrank v-if="showModal && !editMode" :data="selectedItem" @close="showModal=false" @saved="fetchItems"/>
    <edit-talentrank v-if="showModal && editMode" :data="selectedItem" @close="showModal=false" @saved="fetchItems"/>

    <!-- Delete Confirmation Modal -->
    <delete-confirm-modal 
      :visible="deleteModalVisible"
      title="Delete TalentRank"
      message="Are you sure you want to delete this talentRank?"
...
<script>
import AddTalentRank from "./AddTalentRank.vue";
import EditTalentRank from "./EditTalentRank.vue";
import Loading from "@/components/Loading.vue";
import DeleteConfirmModal from "@/components/DeleteConfirmModal.vue";

export default {
  components: { AddTalentRank, EditTalentRank, Loading, DeleteConfirmModal },
  ...
  methods: {
    async fetchItems(page = 1) {
      ...
      try {
        const response = await this.$apiGet('/talentrank', params);
    ...
    viewDetails(id) { 
      this.$router.push({ name: 'TalentRank-detail', params: { id } });
    },
    ...
    // Delete with toast
    async confirmDelete() {
      const res = await this.$apiDelete('/talentrank', this.deleteId);
      if(res) {
        this.$root.$refs.toast.showToast('TalentRank deleted successfully', 'success');
      }

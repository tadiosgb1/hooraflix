...
<template>
  <div class="p-6 bg-black min-h-screen text-sm text-white">
    ...
    <div class="flex items-center justify-between mb-6 border-b border-red-600/30 pb-4">
      <h1 class="text-lg font-bold text-white">Parts Detail</h1>
    </div>
    ...
    <div class="bg-gradient-to-br from-gray-900/50 to-black overflow-hidden rounded-2xl border border-gray-700/50 p-4 shadow-md hidden md:block space-y-2">
      ...
      <div><strong>Series_id:</strong> {{ item.series_id }}</div><div><strong>Title:</strong> {{ item.title }}</div><div><strong>Part_number:</strong> {{ item.part_number }}</div><div><strong>Duration:</strong> {{ item.duration }}</div><div><strong>Vide:file:</strong> {{ item.vide:file }}</div>
    </div>
...
export default {
  ...
  async mounted() {
    ...
    try {
      const response = await this.$apiGetById('/parts', id);
      this.item = response || {};
    } catch (error) {
      console.error(error);
    } finally {

...
<template>
  <div class="p-6 bg-black min-h-screen text-sm text-white relative">
    <!-- Loading -->
    <Loading :visible="loading" message="Loading contentAccessLogs..." />

    <!-- Page Header -->
    <div class="flex items-center justify-between mb-6 border-b border-red-600/30 pb-4">
      <h1 class="text-lg font-bold text-white">ContentAccessLogs</h1>
      <button @click="openAddModal" class="bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-lg font-medium shadow-md flex items-center space-x-1 text-sm transition">
        ...
        <span>Add ContentAccessLogs</span>
      </button>
    </div>
        ...
        <table class="min-w-full text-sm divide-y divide-gray-700/50">
          <thead class="bg-gray-900/80 text-gray-300 uppercase text-xs font-semibold border-b border-red-600/30">
            <tr>
              ...
              <th class="px-6 py-3 text-left">User_id</th><th class="px-6 py-3 text-left">Content_id</th><th class="px-6 py-3 text-left">Device</th><th class="px-6 py-3 text-left">Session_id</th><th class="px-6 py-3 text-left">Ip_address</th><th class="px-6 py-3 text-left">Country</th>
          ...
          <tbody class="divide-y divide-gray-700/50">
            <tr v-for="(item, index) in items" :key="item.id" class="hover:bg-red-900/10 transition duration-150">
              ...
              <td class="px-6 py-4 whitespace-nowrap text-gray-300">{{ item.user_id }}</td><td class="px-6 py-4 whitespace-nowrap text-gray-300">{{ item.content_id }}</td><td class="px-6 py-4 whitespace-nowrap text-gray-300">{{ item.device }}</td><td class="px-6 py-4 whitespace-nowrap text-gray-300">{{ item.session_id }}</td><td class="px-6 py-4 whitespace-nowrap text-gray-300">{{ item.ip_address }}</td><td class="px-6 py-4 whitespace-nowrap text-gray-300">{{ item.country }}</td>
    ...
    <div class="md:hidden space-y-4">
      <div v-for="(item, index) in items" :key="item.id" class="bg-gradient-to-br from-gray-900/50 to-black border border-gray-700/50 rounded-2xl shadow-md p-4">
        <div class="flex justify-between mb-3">
          <h2 class="font-bold text-white">ContentAccessLogs #{{ index + 1 }}</h2>
        ...
        <div class="grid grid-cols-2 gap-y-1 text-sm text-gray-300">
          ...
          <div class="col-span-2">
            <span class="font-medium text-gray-500">Device:</span>
            {{ item.device }}
          </div>
          <div class="col-span-2">
            <span class="font-medium text-gray-500">Session_id:</span>
            {{ item.session_id }}
          </div>
          <div class="col-span-2">
            <span class="font-medium text-gray-500">Ip_address:</span>
            {{ item.ip_address }}
          </div>
          <div class="col-span-2">
    ...
    <!-- Add/Edit Modal -->
    <AddContentAccessLogs v-if="showModal && !editMode" :data="selectedItem" @close="showModal=false" @saved="fetchItems"/>
    <EditContentAccessLogs v-if="showModal && editMode" :data="selectedItem" @close="showModal=false" @saved="fetchItems"/>

    <!-- Delete Confirmation Modal -->
    <delete-confirm-modal 
      :visible="deleteModalVisible"
      title="Delete ContentAccessLogs"
      message="Are you sure you want to delete this contentAccessLogs?"
...
<script>
import AddContentAccessLogs from "./AddContentAccessLogs.vue";
import EditContentAccessLogs from "./EditContentAccessLogs.vue";
import Loading from "@/components/Loading.vue";
import DeleteConfirmModal from "@/components/DeleteConfirmModal.vue";

export default {
  components: { AddContentAccessLogs, EditContentAccessLogs, Loading, DeleteConfirmModal },
  ...
  methods: {
    async fetchItems(page = 1) {
      ...
      try {
        const response = await this.$apiGet('/contentaccesslogs', params);
    ...
    viewDetails(id) { 
      this.$router.push({ name: 'ContentAccessLogs-detail', params: { id } });
    },
    ...
    // Delete with toast
    async confirmDelete() {
      const res = await this.$apiDelete('/contentaccesslogs', this.deleteId);
      if(res) {
        this.$root.$refs.toast.showToast('ContentAccessLogs deleted successfully', 'success');
      }

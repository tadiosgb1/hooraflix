...
<template>
  <div class="p-6 bg-black min-h-screen text-sm text-white">
    ...
    <div class="flex items-center justify-between mb-6 border-b border-red-600/30 pb-4">
      <h1 class="text-lg font-bold text-white">ContentAccessLogs Detail</h1>
    </div>
    ...
    <div class="bg-gradient-to-br from-gray-900/50 to-black overflow-hidden rounded-2xl border border-gray-700/50 p-4 shadow-md hidden md:block space-y-2">
      ...
      <div><strong>Device:</strong> {{ item.device }}</div>
      <div><strong>Session_id:</strong> {{ item.session_id }}</div>
      <div><strong>Ip_address:</strong> {{ item.ip_address }}</div>
...
export default {
  ...
  async mounted() {
    ...
    try {
      const response = await this.$apiGetById('/contentaccesslogs', id);
      this.item = response || {};
    } catch (error) {
      console.error(error);
    } finally {

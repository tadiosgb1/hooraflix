...
<template>
  <div class="p-6 bg-gray-50 min-h-screen text-sm text-gray-800 relative">
    ...
    <div class="flex items-center justify-between mb-6 border-b pb-4 border-gray-200">
      <h1 class="text-lg font-bold text-gray-800">LiveEvent</h1>
        ...
        <table class="min-w-full text-sm divide-y divide-gray-200">
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-for="(item, index) in items" :key="item.id" class="hover:bg-green-50 transition duration-150">
              ...
              <td class="px-6 py-4 whitespace-nowrap">{{ item.title }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.host_talent_id }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.scheduled_at }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.stream_key }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.status(upcoming,live,ended) }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.is_ppv }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.ticket_price }}</td>
    ...
    <div class="md:hidden space-y-4">
      <div v-for="(item, index) in items" :key="item.id" class="bg-white border border-gray-200 rounded-xl shadow p-4">
        <div class="flex justify-between mb-3">
          <h2 class="font-bold text-gray-800">LiveEvent #{{ index + 1 }}</h2>
        ...
        <div class="grid grid-cols-2 gap-y-1 text-sm text-gray-700">
            ...
            <div class="col-span-2">
              <span class="font-medium text-gray-600">Host_talent_id:</span>
              {{ item.host_talent_id }}
            </div>
            <div class="col-span-2">
              <span class="font-medium text-gray-600">Scheduled_at:</span>
              {{ item.scheduled_at }}
            </div>
            <div class="col-span-2">
              <span class="font-medium text-gray-600">Stream_key:</span>
              {{ item.stream_key }}
            </div>
            <div class="col-span-2">
              <span class="font-medium text-gray-600">Status(upcoming,live,ended):</span>
              {{ item.status}}
            </div>
            <div class="col-span-2">
              <span class="font-medium text-gray-600">Is_ppv:</span>
              {{ item.is_ppv }}
            </div>
            <div class="col-span-2">
              <span class="font-medium text-gray-600">Ticket_price:</span>
              {{ item.ticket_price }}
            </div>
        </div>
      </div>

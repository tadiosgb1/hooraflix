...
<template>
  <div class="p-6 bg-black min-h-screen text-sm text-white relative">
    ...
    <div class="flex items-center justify-between mb-6 border-b border-red-600/30 pb-4">
      <h1 class="text-lg font-bold text-white">LiveEvent</h1>
        ...
        <table class="min-w-full text-sm divide-y divide-gray-200">
          <thead class="bg-gray-900/80 text-gray-300 uppercase text-xs font-semibold border-b border-red-600/30">
            <tr>
              ...
              <th class="px-6 py-3 text-left">Title</th><th class="px-6 py-3 text-left">Host_talent_id</th><th class="px-6 py-3 text-left">Scheduled_at</th><th class="px-6 py-3 text-left">Stream_key</th><th class="px-6 py-3 text-left">Status</th><th class="px-6 py-3 text-left">Is_ppv</th><th class="px-6 py-3 text-left">Ticket_price</th>
          ...
          <tbody class="divide-y divide-gray-700/50">
            <tr v-for="(item, index) in items" :key="item.id" class="hover:bg-red-900/10 transition duration-150">
              ...
              <td class="px-6 py-4 whitespace-nowrap text-gray-300">{{ item.title }}</td><td class="px-6 py-4 whitespace-nowrap text-gray-300">{{ item.host_talent_id }}</td><td class="px-6 py-4 whitespace-nowrap text-gray-300">{{ item.scheduled_at }}</td><td class="px-6 py-4 whitespace-nowrap text-gray-300">{{ item.stream_key }}</td><td class="px-6 py-4 whitespace-nowrap text-gray-300">{{ item.status(upcoming,live,ended) }}</td><td class="px-6 py-4 whitespace-nowrap text-gray-300">{{ item.is_ppv }}</td><td class="px-6 py-4 whitespace-nowrap text-gray-300">{{ item.ticket_price }}</td>
    ...
    <div class="md:hidden space-y-4">
      <div v-for="(item, index) in items" :key="item.id" class="bg-gradient-to-br from-gray-900/50 to-black border border-gray-700/50 rounded-2xl shadow-md p-4">
        <div class="flex justify-between mb-3">
          <h2 class="font-bold text-white">LiveEvent #{{ index + 1 }}</h2>
        ...
        <div class="grid grid-cols-2 gap-y-1 text-sm text-gray-300">
            ...
            <div class="col-span-2">
              <span class="font-medium text-gray-500">Host_talent_id:</span>
              {{ item.host_talent_id }}
            </div>
            <div class="col-span-2">
              <span class="font-medium text-gray-500">Scheduled_at:</span>
              {{ item.scheduled_at }}
            </div>
            <div class="col-span-2">
              <span class="font-medium text-gray-500">Stream_key:</span>
              {{ item.stream_key }}
            </div>
            <div class="col-span-2">
              <span class="font-medium text-gray-500">Status(upcoming,live,ended):</span>
              {{ item.status}}
            </div>
            <div class="col-span-2">
              <span class="font-medium text-gray-500">Is_ppv:</span>
              {{ item.is_ppv }}
            </div>
            <div class="col-span-2">
              <span class="font-medium text-gray-500">Ticket_price:</span>
              {{ item.ticket_price }}
            </div>
        </div>
      </div>
    ...
    <!-- Add/Edit Modal -->
    <add-liveevent v-if="showModal && !editMode" :data="selectedItem" @close="showModal=false" @saved="fetchItems"/>
    <edit-liveevent v-if="showModal && editMode" :data="selectedItem" @close="showModal=false" @saved="fetchItems"/>

    <!-- Delete Confirmation Modal -->
    <delete-confirm-modal 
      :visible="deleteModalVisible"
      title="Delete LiveEvent"
      message="Are you sure you want to delete this liveEvent?"
...
<script>
import AddLiveEvent from "./AddLiveEvent.vue";
import EditLiveEvent from "./EditLiveEvent.vue";
import Loading from "@/components/Loading.vue";
import DeleteConfirmModal from "@/components/DeleteConfirmModal.vue";

export default {
  components: { AddLiveEvent, EditLiveEvent, Loading, DeleteConfirmModal },
  ...
  methods: {
    async fetchItems(page = 1) {
      ...
      try {
        const response = await this.$apiGet('/liveevent', params);
    ...
    viewDetails(id) { 
      this.$router.push({ name: 'LiveEvent-detail', params: { id } });
    },
    ...
    // Delete with toast
    async confirmDelete() {
      const res = await this.$apiDelete('/liveevent', this.deleteId);
      if(res) {
        this.$root.$refs.toast.showToast('LiveEvent deleted successfully', 'success');
      }

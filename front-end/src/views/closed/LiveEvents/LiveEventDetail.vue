...
<template>
  <div class="p-6 bg-black min-h-screen text-sm text-white">
    ...
    <div class="bg-gradient-to-br from-gray-900/50 to-black overflow-hidden rounded-2xl border border-gray-700/50 p-4 shadow-md hidden md:block space-y-2">
      ...
      <div><strong>Title:</strong> {{ item.title }}</div><div><strong>Host_talent_id:</strong> {{ item.host_talent_id }}</div><div><strong>Scheduled_at:</strong> {{ item.scheduled_at }}</div><div><strong>Stream_key:</strong> {{ item.stream_key }}</div><div><strong>Status:</strong> {{ item.status }}</div><div><strong>Is_ppv:</strong> {{ item.is_ppv }}</div><div><strong>Ticket_price:</strong> {{ item.ticket_price }}</div>
    </div>
